...
with open('.\\raw_data\\new_data\\passmedicine_part3.txt', 'r', encoding="utf-8") as f:
    text = f.read()
...
for i, paragraph in enumerate(paragraphs):
    ...
    choices_formatted = [choice[2:-1] for choice in choices]
    print(choices)
    print(choices_formatted)

    for choice in choices:
        print("choice: ", choice)
        paragraphs[i] = paragraphs[i].replace(choice, choice[:2] + " " + choice[2:])

    answer = re.search('Answer:\w.*\n', paragraph)
    if answer:
        print(answer.group())
        answer_formated = answer.group()[7:-1]
...
with open('.\\processed_data\\text\\passmedicine_part3_processed.txt', 'w', encoding="utf-8") as f:
